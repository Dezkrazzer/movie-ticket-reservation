...
public class MovieController {
    ...
    @GetMapping("/filminfo")
    public String showFilmInfoByTitle(@RequestParam("title") String title, Model model) {
        // Cari film berdasarkan judul (case-insensitive)
        Movie movie = MovieData.getMovieList().stream()
                .filter(m -> m.getJudulFilm().equalsIgnoreCase(title))
                .findFirst()
                .orElse(null);

        if (movie == null) {
            return "redirect:/404"; // Jika tidak ditemukan
        }

        // Tambahkan dummy reviews
        List<Map<String, String>> reviews = new ArrayList<>();
        reviews.add(Map.of("nama", "Ani", "komentar", "Filmnya keren! Recomended untuk ditonton!"));
        reviews.add(Map.of("nama", "Budi", "komentar", "Dari skala nol hingga lima, saya memberi film ini nilai lima."));
        reviews.add(Map.of("nama", "Citra", "komentar", "Menyenangkan dan menghibur, cocok untuk ditonton bersama teman & keluarga."));

        model.addAttribute("movie", movie);
        model.addAttribute("reviews", reviews);
        model.addAttribute("ratingStars", "★★★★☆");
